...
from cli.keys import get_test_decryption_keys
...
DIMENSION_ACTIONS = {
    "coding": "Tighten your edit/verify loop: after each file change run a quick local check, then a full verify before exiting.",
    "memory": "Implement a durable state file with explicit schema + versioning, read it on startup, and checkpoint after each mutation.",
    "tool-use": "Wrap every tool call with retry/backoff, schema validation, and fallback handling for malformed or partial responses.",
    "planning": "Add an explicit planner/executor split: write a step plan, track completion state, and replan when constraints change.",
    "resilience": "Add checkpoint/resume and idempotent writes so restarts and partial failures do not reprocess completed work.",
    "safety": "Enforce path/input allowlists and redact secrets before logging or output serialization.",
    "multi-agent": "Use file locks or optimistic concurrency with conflict retries when writing shared state.",
}

PATTERN_ACTION_RULES = [
    {
        "id": "rate_limit",
        "area": "tool-use",
        "patterns": ["429", "retry-after", "rate limit", "quota"],
        "action": "Implement a shared retry policy: parse Retry-After, exponential backoff with jitter, and batched requests where available.",
    },
    {
        "id": "timeout",
        "area": "resilience",
        "patterns": ["timed out", "timeout"],
        "action": "Add checkpointing and chunked processing so long tasks can resume without starting over.",
    },
    {
        "id": "audit",
        "area": "tool-use",
        "patterns": ["audit log", "signature mismatch", "seq=", "nonce"],
        "action": "Instrument tool calls consistently and preserve audit artifacts in setup/ so verification can replay call history.",
    },
    {
        "id": "missing_output",
        "area": "memory",
        "patterns": ["does not exist", "missing", "not found"],
        "action": "Add explicit output contracts: write required files atomically and validate existence/content before finishing.",
    },
    {
        "id": "tamper",
        "area": "safety",
        "patterns": ["tampered", "scope violation"],
        "action": "Restrict writes to setup/ only and block edits to tests/, tools/, and metadata files.",
    },
]
...
@click.option(
    ...
    "-t",
    default=120,
    help="Timeout per challenge in seconds (default: 120)",
)
def score(
    challenge_id: str,
    workspace: str | None,
    ...
    for i, cid in enumerate(ids, 1):
        ...
        if not challenge_ws.exists():
            click.echo(
                f"  [{i}/{len(ids)}] {cid}: not fetched yet. Run 'opengym fetch {cid}' first.",
                err=True,
            )
            continue

        result = score_challenge(
            cid, challenge_ws, challenges_dir / cid, timeout=timeout
        )
        ...
        if not json_output and not csv_output and not summary:
            ...
            click.echo(
                f"  [{i}/{len(ids)}] {cid}: {status} ({result['tests_passed']}/{result['tests_total']} tests)"
            )
...
def score_challenge(
    ...
    with tempfile.TemporaryDirectory(prefix=f"opengym-score-{challenge_id}-") as tmp:
        ...
        _prepare_scoring_workspace(workspace_path, original_path, staging_path)

        # Check tool script integrity — agent must not modify tool scripts.
        tools_tampered = check_tool_integrity(
            staging_path / "tools", original_path / "tools"
        )
        if tools_tampered:
            return {
                "challenge": challenge_id,
        ...
        if is_pytest:
            tests_passed, tests_total, test_details = parse_pytest_output(
        ...
        challenge_score = (
            round((tests_passed / tests_total) * 100) if tests_total > 0 else 0
        )
...
def _prepare_scoring_workspace(
    workspace_path: Path, original_path: Path, staging_path: Path
):
    """Build an isolated scoring workspace with decrypted tests."""
    shutil.copytree(workspace_path, staging_path)

    # Ignore any user-created tests and always stage canonical hidden tests.
    staged_tests = staging_path / "tests"
    if staged_tests.exists():
        shutil.rmtree(staged_tests)
    if (original_path / "tests").exists():
        _inject_tests(original_path / "tests", staged_tests)

    # Ensure audit helpers are available for verify scripts that import _audit.
    tools_dir = staging_path / "tools"
    audit_src = Path(__file__).resolve().parent.parent / "lib" / "audit.py"
    if tools_dir.exists() and audit_src.exists():
        shutil.copy2(audit_src, tools_dir / "_audit.py")


def _decrypt_with_known_keys(ciphertext: bytes) -> bytes:
    """Attempt decrypting with all configured candidate keys."""
    last_error = None
    for key in get_test_decryption_keys():
        try:
    ...
    if last_error:
        raise last_error
    raise ValueError("No test decryption keys available.")
...
def _inject_tests(original_tests: Path, workspace_tests: Path):
    ...
    for item in original_tests.iterdir():
        ...
        if item.name.startswith("__"):
            continue
        if item.suffix == ".enc":
            plaintext = _decrypt_with_known_keys(item.read_bytes())
...
def check_test_integrity(workspace_tests: Path, original_tests: Path) -> bool:
    ...
    for original_file in original_tests.rglob("*"):
        ...
        if original_file.suffix == ".enc":
            ...
            orig_content = _decrypt_with_known_keys(original_file.read_bytes())
        else:
...
def _collect_failure_text(results: list[dict]) -> list[str]:
    """Collect error and failed-test messages for pattern-based remediation."""
    text_blobs: list[str] = []
    for result in results:
        if result.get("error"):
            text_blobs.append(str(result["error"]))
        if result.get("output"):
            text_blobs.append(str(result["output"]))
        for test in result.get("failed_tests", []):
            message = test.get("message", "")
            if message:
                text_blobs.append(str(message))
    return text_blobs


def build_action_plan(
    results: list[dict], dimension_scores: dict[str, int]
) -> list[dict]:
    """Build concrete remediation actions from score and failure patterns."""
    actions: list[dict] = []
    seen: set[str] = set()

    # 1) Always include weak-dimension actions first.
    for dim, score in sorted(dimension_scores.items(), key=lambda item: item[1]):
        if score >= 70 or dim not in DIMENSION_ACTIONS:
            continue
        actions.append(
            {
                "area": dim,
                "reason": f"score {score}/100",
                "action": DIMENSION_ACTIONS[dim],
            }
        )
        seen.add(dim)

    # 2) Add pattern-derived actions from actual failures.
    failure_text = "\n".join(_collect_failure_text(results)).lower()
    for rule in PATTERN_ACTION_RULES:
        if rule["id"] in seen:
            continue
        if any(pattern in failure_text for pattern in rule["patterns"]):
            actions.append(
                {
                    "area": rule["area"],
...
def build_summary(results: list[dict]) -> dict:
    ...
    total_score = (
        round(sum(r["score"] for r in results) / len(results)) if results else 0
    )
    ...
    for cat, cat_score in category_scores.items():
        if cat_score < 70 and cat in CATEGORY_SUGGESTIONS:
            suggestions.append(
                f"  {cat} ({cat_score}/100): {CATEGORY_SUGGESTIONS[cat]}"
            )
    ...
    action_plan = build_action_plan(results, dimension_scores)
...
def build_scorecard(results: list[dict]) -> dict:
    ...
    """
    infra_results = [
        r for r in results if r.get("challenge_type") == "INFRA_CONFORMANCE"
    ]
    model_results = [
        r for r in results if r.get("challenge_type") != "INFRA_CONFORMANCE"
    ]
    ...
    infra_score = (
        round(sum(r["score"] for r in infra_results) / len(infra_results))
        if infra_results
        else 0
    )
    ...
    model_score = (
        round(sum(r["score"] for r in model_results) / len(model_results))
        if model_results
        else 0
    )
...
def print_scorecard(report: dict):
    ...
    click.echo(
        f"  Infra Conformance:  {report['infra_score']}/100  ({report['infra_passed']}/{report['infra_total']} passed)",
        err=True,
    )
    if report["model_total"] > 0:
        click.echo(
            f"  Model-Dependent:    {report['model_score']}/100  ({report['model_passed']}/{report['model_total']} passed)",
            err=True,
        )
    ...
    for dim, info in sorted(
        report["by_dimension"].items(), key=lambda x: -x[1]["score"]
    ):
        bar = "#" * (info["score"] // 5) + "." * (20 - info["score"] // 5)
        click.echo(
            f"    {dim:<14} [{bar}] {info['score']:>3}/100  ({info['passed']}/{info['total']})",
            err=True,
        )
    ...
    for cat, info in sorted(report["by_category"].items(), key=lambda x: x[1]["score"]):
        ...
        click.echo(
            f"    {label:<28} [{bar}] {score:>3}/100  {passed}/{total}  {status}",
            err=True,
        )
...
def print_summary(report: dict):
    ...
    click.echo(
        f"  Passed: {report['passed']}/{report['challenges_attempted']}", err=True
    )
    duration = report.get("total_duration_seconds", 0)
    cost = report.get("total_estimated_cost_usd", 0)
    if duration > 0:
    ...
    if action_plan:
        ...
        for item in action_plan:
            click.echo(
                f"  - {item['area']} ({item['reason']}): {item['action']}",
                err=True,
            )
